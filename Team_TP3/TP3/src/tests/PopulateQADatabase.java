...
public class PopulateQADatabase {
	...
	public void execute() {
		...
		question = new Question("User Stories for HW2",
				"Where are the user stories for HW2 located? Are they the same ones we were working on for TP1?", 5);
		try {
		...
		question = new Question("How many UML classes do we need to make HW2?",
				"How many UML diagrams do we need to make for HW2, it is one per class right? So four in total?", 1);
		try {
			qaHelper.registerQuestion(question);
		} catch (SQLException e) {
		...
		// Populate Answer 3
		answer = new Answer("Makes sense to me.", 5);
		try {
		...
		// Populate Answer 5
		answer = new Answer("The live event is Tuesday at 7 PM MST.", 15);
		try {
		...
		question = new Question("HW2 Is Astah required?",
				"Is Astah required for HW2 or can we use something like draw.io instead?", 2);
		try {
			qaHelper.registerQuestion(question);
		} catch (SQLException e) {
		...
		// Populate Answer 7
		answer = new Answer("Does the word \"Astah\" appear in the assignment?", 3);
		try {
		...
		question = new Question("Understanding of HW2", "Hi all, \r\n" + "\r\n"
				...
				+ "If you're Staff and notice something is wrong with my interpretation, please let me know!", 4);
		try {
			qaHelper.registerQuestion(question);
		} catch (SQLException e) {
		...
		question = new Question("Document not showing on HW2", "Hi all, \r\n" + "\r\n"
				...
				+ "\r\n" + "Is it just my canvas or do others see this too?", 6);
		try {
			qaHelper.registerQuestion(question);
		} catch (SQLException e) {
		...
		answer = new Answer("Thats a great response!!!", 9);
		try {
			qaHelper.registerAnswerWithQuestion(answer, 2); // Register answer in relation to question id 2 for the
															// relation table
		} catch (SQLException e) {
		...
		answer = new Answer("Thank you so much!!!", 10);
		try {
			qaHelper.registerAnswerWithQuestion(answer, 2); // Register answer in relation to question id 2 for the
															// relation table
		} catch (SQLException e) {
		...
		answer = new Answer("Woweeeeeeee", 11);
		try {
			qaHelper.registerAnswerWithQuestion(answer, 4); // Register answer in relation to question id 4 for the
															// relation table
		} catch (SQLException e) {
		...
		answer = new Answer("Well that raises some interesting questions.", 12);
		try {
			qaHelper.registerAnswerWithQuestion(answer, 5); // Register answer in relation to question id 5 for the
															// relation table
		} catch (SQLException e) {
		...
		// Populate some Reviews for Questions
		
		// Populate Review 1
				review = new Review(true, 1, "This is a test review for question 1", 1);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 1");
					e.printStackTrace();
				}
				
				// Populate Review 2
				review = new Review(true, 2, "This is a test review for question 2", 2);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 2");
					e.printStackTrace();
				}
				
				// Populate Review 3
				review = new Review(true, 3, "This is a test review for question 3", 3);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 3");
					e.printStackTrace();
				}
				
				// Populate Review 4
				review = new Review(true, 4, "This is a test review for question 4", 4);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 4");
					e.printStackTrace();
				}
				
				// Populate Review 5
				review = new Review(true, 5, "This is a test review for question 5", 5);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 5");
					e.printStackTrace();
				}
		
		// Populate some Reviews for Answers
		
				// Populate Review 6
				review = new Review(false, 1, "This is a test review for answer 1", 6);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 6");
					e.printStackTrace();
				}
				
				// Populate Review 7
				review = new Review(false, 2, "This is a test review for answer 2", 7);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 7");
					e.printStackTrace();
				}
				
				// Populate Review 8
				review = new Review(false, 3, "This is a test review for answer 3", 8);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 8");
					e.printStackTrace();
				}
				
				// Populate Review 9
				review = new Review(false, 4, "This is a test review for answer 4", 9);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 9");
					e.printStackTrace();
				}
				
				// Populate Review 10
				review = new Review(false, 5, "This is a test review for answer 5", 10);
				try {
					qaHelper.registerReview(review);
				} catch (SQLException e) {
					System.out.println("Error registering review 10");
					e.printStackTrace();
				}
	}
