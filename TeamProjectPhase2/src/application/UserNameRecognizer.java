	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 1:
				...
				// A-Z, a-z, 0-9 transition to State 1
				if ((currentChar >= 'A' && currentChar <= 'Z') || // Check for A-Z
						(currentChar >= 'a' && currentChar <= 'z') || // Check for a-z
				...
				// period, minus or underscore transition to State 2
			...
			case 2:
				// State 2 deals with a character after a period, minus or underscore in the
				// name.
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += "A UserName character may only contain the characters A-Z, a-z, 0-9 or period, "
						+ "minus, or underscore.\n";
				return userNameRecognizerErrorMessage;
			} else {
				// UserName is valid
